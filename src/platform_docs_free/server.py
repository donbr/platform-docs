#!/usr/bin/env python3
"""
Platform Docs MCP Server (FastEmbed) - Free semantic search across developer documentation.

This MCP server provides semantic search over documentation pages using
FastEmbed embeddings (BAAI/bge-small-en-v1.5) - completely free, no OpenAI API key needed!

Collection: platform-docs-fastembed
"""
...
mcp = FastMCP("platform-docs-fastembed")
...
def get_vector_store() -> QdrantVectorStore:
    """
    Initialize and return Qdrant vector store with FastEmbed embeddings.

    No OpenAI API key required - uses free local model.
    """
...
def search_docs(query: str, k: int = DEFAULT_K, source: Optional[str] = None) -> str:
    """
    Search documentation using semantic similarity (FastEmbed - free).
    ...
    This server uses FastEmbed (BAAI/bge-small-en-v1.5) - no OpenAI API key needed!
    ...
    try:
        ...
        try:
            ...
            if source and "Index required" in str(filter_error):
                ...
                if not results:
                    return f"No results found for query '{query}' in source '{source}'."
            else:
                raise
        ...
        output = [f"Found {len(results)} results for: '{query}' (FastEmbed)\n"]
...
def list_sources() -> str:
    ...
    try:
        ...
        output = ["Available Documentation Sources (FastEmbed collection):\n"]
        ...
        output.append("\nNote: This collection uses FastEmbed (free, no API key)")
